...
# Access the transfer schedule information
output "snapmirror_schedule" {
  value = data.netapp-ontap_snapmirror.snapmirror.policy.transfer_schedule.name
}
...
- `transfer_schedule` (Attributes) Transfer schedule details (see [below for nested schema](#nestedatt--policy--transfer_schedule))

<a id="nestedatt--policy--transfer_schedule"></a>

### Nested Schema for `policy.transfer_schedule`
