...
# Access the transfer schedule information for the first snapmirror
output "first_snapmirror_schedule" {
  value = data.netapp-ontap_snapmirrors.snapmirrors.snapmirrors[0].policy.transfer_schedule.name
}
...
- `transfer_schedule` (Attributes) Transfer schedule details (see [below for nested schema](#nestedatt--snapmirrors--policy--transfer_schedule))

<a id="nestedatt--snapmirrors--policy--transfer_schedule"></a>

### Nested Schema for `snapmirrors.policy.transfer_schedule`
