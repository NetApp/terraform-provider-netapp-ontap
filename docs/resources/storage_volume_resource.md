...
# netapp-ontap_storage_volume (Resource)
...
For example
```shell
 terraform import netapp-ontap_storage_volume.example vol1,svm2,cluster5
```
...
```terraform
import {
  to = netapp-ontap_storage_volume.volume_import
...
resource "netapp-ontap_storage_volume" "volume_import" {
  aggregates = [
    {
      name = "aggr1"
    },
