...
- `constituents_per_aggregate` (Number) Number of constituents per aggregate when creating a FlexGroup volume. If specified on a single aggregate, creates a FlexGroup volume instead of a flexible volume
...
- `style` (String) Volume style (flexvol, flexgroup, flexgroup_constituent). Specifies the volume type to create
