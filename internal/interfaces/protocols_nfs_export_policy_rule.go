...
func CreateExportPolicyRule(errorHandler *utils.ErrorHandler, r restclient.RestClient, data ExportpolicyRuleResourceModel, exportPolicyID string) (*ExportPolicyRuleGetDataModelONTAP, error) {
...
func GetExportPolicyRule(errorHandler *utils.ErrorHandler, r restclient.RestClient, exportPolicyID string, index int64) (*ExportPolicyRuleGetDataModelONTAP, error) {
	statusCode, response, err := r.GetNilOrOneRecord("protocols/nfs/export-policies/"+exportPolicyID+"/rules/"+strconv.FormatInt(index, 10), nil, nil)
	if err != nil {
...
func UpdateExportPolicyRule(errorHandler *utils.ErrorHandler, r restclient.RestClient, data ExportpolicyRuleResourceModel, exportPolicyID string, index int64) (*ExportPolicyRuleGetDataModelONTAP, error) {
	var body map[string]interface{}
	if err := mapstructure.Decode(data, &body); err != nil {
		return nil, errorHandler.MakeAndReportError("error encoding export policy rule body", fmt.Sprintf("error on encoding export policy rule body: %s, body: %#v", err, data))
	}
