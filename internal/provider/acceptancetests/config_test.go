...
func TestConfig_GetConnectionProfile(t *testing.T) {
	...
	cxProfile := provider.ConnectionProfile{}
	cxProfiles := map[string]provider.ConnectionProfile{"empty": cxProfile}
	cxProfilesTwo := map[string]provider.ConnectionProfile{"empty1": cxProfile, "empty2": cxProfile}
	tests := []struct {
		name    string
		fields  fields
		args    args
		want    *provider.ConnectionProfile
		wantErr bool
	}{
		...
		{name: "test_no_profiles", fields: fields{ConnectionProfiles: map[string]provider.ConnectionProfile{}, Version: "v1.2.3"}, args: args{name: "other"}, want: nil, wantErr: true},
