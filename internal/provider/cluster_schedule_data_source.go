...
func (d *ClusterScheduleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if restInfo.Type == "cron" {
		data.Cron = &CronScheduleModel{
			Minutes:  flattenTypesInt64List(restInfo.Cron.Minutes),
			Hours:    flattenTypesInt64List(restInfo.Cron.Hours),
			Days:     flattenTypesInt64List(restInfo.Cron.Days),
			Weekdays: flattenTypesInt64List(restInfo.Cron.Weekdays),
			Months:   flattenTypesInt64List(restInfo.Cron.Months),
		}
	} else {
