...
func TestAccNameServicesLDAPResource(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Test create
			{
				Config: testAccNameServicesLDAPResourceConfig("tf_acc_svm", "subtree"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("netapp-ontap_name_services_ldap.name_services_ldap", "svm_name", "tf_acc_svm"),
					resource.TestCheckResourceAttr("netapp-ontap_name_services_ldap.name_services_ldap", "servers.0", "1.1.1.1"),
				),
			},
			// Test update
			{
				Config: testAccNameServicesLDAPResourceConfig("tf_acc_svm", "base"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("netapp-ontap_name_services_ldap.name_services_ldap", "svm_name", "tf_acc_svm"),
