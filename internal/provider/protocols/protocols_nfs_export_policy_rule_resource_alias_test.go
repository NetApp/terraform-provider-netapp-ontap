...
func TestAccNFSExportPolicyRuleResourceAlias(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccNFSExportPolicyRuleResourceConfigAlias("tf_acc_svm", "default"),
				Check: resource.ComposeTestCheckFunc(
					...
					// check id
					resource.TestMatchResourceAttr("netapp-ontap_protocols_nfs_export_policy_rule_resource.example1", "id", regexp.MustCompile(`cluster4_tf_acc_svm_default_`)),
				),
			},
			// update test
			{
				Config: testAccNFSExportPolicyRuleResourceConfigAliasUpdateProtocolsROrule("tf_acc_svm", "default"),
				Check: resource.ComposeTestCheckFunc(
					// check default values
					resource.TestCheckResourceAttr("netapp-ontap_protocols_nfs_export_policy_rule_resource.example1", "allow_suid", "true"),
			...
			// Test importing a resource
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("netapp-ontap_protocols_nfs_export_policy_rule_resource.example1", "svm_name", "tf_acc_svm"),
