...
func TestAccStorageVolumeSnapshotResource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccStorageVolumeSnapshotResourceConfig("tf_acc_svm", "my comment"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "volume_name", "tf_acc_volume"),
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "name", "snaptest"),
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "svm_name", "tf_acc_svm"),
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "comment", "my comment"),
				),
			},
			// Update and read testing
			{
				Config: testAccStorageVolumeSnapshotResourceConfig("tf_acc_svm", "new comment"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "volume_name", "tf_acc_volume"),
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "name", "snaptest"),
					resource.TestCheckResourceAttr("netapp-ontap_volume_snapshot.example", "svm_name", "tf_acc_svm"),
